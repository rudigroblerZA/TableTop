...
| `check-settings-defaults.py` | an `IAppSettings` property whose default differs between the three heads | `AutoNextPlayer` was ported into WinUI *because* the heads behaved differently, and arrived defaulting to `true` while MAUI and Android both said `false`. The interface constrains the shape, not the first-run value; `TableTop.Tests` cannot reference a head, and the test doubles pick their own values |
